...
func (x *receiver) deleteUpload(c echo.Context) error {
	uploadID := c.Param("id")
	if uploadID == "" {
		return echo.NewHTTPError(http.StatusBadRequest, "upload_id is required")
	}

	uploadDir := filepath.Join(x.dataDir, uploadID)
...
func (x *receiver) list(c echo.Context) error {
	entries, err := os.ReadDir(x.dataDir)
	if err != nil {
...
func (x *receiver) upload(c echo.Context) error {
